<script>
(function(){
  const path = location.pathname.replace(/\/+$/,'/');
  const segs = path.split('/').filter(Boolean);

  let lang='en', section='alarch';
  if (segs.length===0) { lang='en'; section='alarch'; }
  else if (segs.length===1 && (segs[0]==='en'||segs[0]==='tr')) { lang=segs[0]; section='alarch'; } // /en/ veya /tr/
  else if (segs.length===1) { section=segs[0]; }
  else { lang=segs[0]; section=segs[1]; }

  if (section === 'alog') section = 'allog';
  const SECTIONS = ['alarch','alart','allog','alo'];
  if (!SECTIONS.includes(section)) section = 'alarch';
  if (!['en','tr'].includes(lang)) lang = 'en';

  function qs(s){ return document.querySelector(s); }
  function setText(sel, txt){ const n=qs(sel); if(n) n.textContent=txt; }
  function show(sel, on){ const n=qs(sel); if(!n) return; n.classList.toggle('hide', !on); }
  function linkFor(sec){ return (lang==='tr'? 'tr/':'') + sec + '/'; }       // başına / koymuyoruz
  function langLink(target){ return (target==='tr'? 'tr/':'') + section + '/'; }

  function titleFallback(sec, lang){
    const map = { en:{alarch:'Architecture', alart:'Art', allog:'Log', alo:'Alo'},
                  tr:{alarch:'Mimarlık',     alart:'Sanat', allog:'Günlük', alo:'Alo'} };
    return (map[lang]||map.en)[sec] || sec.toUpperCase();
  }

  // Nav ve dil anahtarı hemen çizilsin (UI boş kalmasın)
  const nav = qs('#js-nav');
  if (nav){
    nav.innerHTML = '';
    SECTIONS.forEach(sec=>{
      const a = document.createElement('a');
      a.href = linkFor(sec); a.textContent = sec.toUpperCase();
      if (sec===section) a.classList.add('active');
      nav.appendChild(a);
    });
  }
  const l = qs('#js-lang');
  if (l){
    l.innerHTML = '';
    ['en','tr'].forEach(L=>{
      const a = document.createElement('a');
      a.href = langLink(L); a.textContent = L.toUpperCase();
      if (L===lang) a.classList.add('on');
      l.appendChild(a);
    });
  }

  // İçeriği çek
  fetch('assets/content.json?ts=' + Date.now())
    .then(r=> r.ok ? r.json() : Promise.reject('content.json not found'))
    .then(data=>{
      const site = data._site || {};
      setText('#js-brand', site.brand || 'AL');

      setText('#js-title', (data[lang]?.[section]?.title) || titleFallback(section, lang));
      const sub = (data[lang]?.[section]?.subtitle) || '';
      setText('#js-sub', sub); show('#js-sub', !!sub);

      const textHtml = (data[lang]?.[section]?.text) || '';
      const rt = qs('#js-rich'); if (rt){ rt.innerHTML = textHtml; show('#js-rich', !!textHtml); }

      const ql = site.quick_links || [];
      const qWrap = qs('#js-quick'); if (qWrap){ qWrap.innerHTML=''; ql.forEach(it=>{ const a=document.createElement('a'); a.href=it.href; a.target='_blank'; a.rel='noopener'; a.textContent=it.label; qWrap.appendChild(a); }) }

      setText('#js-note', lang==='tr' ? (site.note_tr||'') : (site.note_en||''));

      const list = (data[lang]?.[section]?.projects) || [];
      const grid = qs('#js-grid');
      if (grid){
        grid.innerHTML='';
        if (list.length){
          show('#js-grid', true);
          list.forEach(item=>{
            const fig = document.createElement('figure'); fig.className='card';
            const box = document.createElement('div'); box.className='lb';
            let media;
            if ((item.type||'image')==='video'){
              media = document.createElement('video');
              media.src = item.src; media.autoplay=true; media.muted=true; media.loop=true;
              media.addEventListener('mouseover',()=>media.pause());
              media.addEventListener('mouseout',()=>media.play());
            } else {
              media = document.createElement('img'); media.src=item.src; media.alt=item.alt||'';
            }
            box.appendChild(media); fig.appendChild(box);
            if (item.caption){ const c=document.createElement('figcaption'); c.className='cap'; c.textContent=item.caption; fig.appendChild(c); }
            grid.appendChild(fig);
          });
        } else {
          show('#js-grid', false);
        }
      }

      const adBox = qs('#js-ad');
      if (adBox){
        const ad = site.ad && site.ad.enabled ? site.ad.html : '';
        if (ad){ adBox.innerHTML = ad; show('#js-ad', true); } else { show('#js-ad', false); }
      }
    })
    .catch(err=>{
      // Sayfa boş kalmasın diye küçük uyarı
      setText('#js-title', titleFallback(section, lang));
      const rt = qs('#js-rich');
      if (rt){ rt.innerHTML = '<p style="color:#888;font-size:12px">assets/content.json bulunamadı veya yüklenemedi.</p>'; show('#js-rich', true); }
      console.warn(err);
    });
})();
</script>
